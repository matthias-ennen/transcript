...
class ModelDownloadService : Service() {
        ...
        downloadJob = serviceScope.launch {
            runCatching { downloadAndVerify(model) }
                .onSuccess {
                    ...
                    finish(model, "Download abgeschlossen", "${model.modelLabel} ist installiert.")
                }
                .onFailure { failure ->
                    clearActiveModel()
                    ...
                    val message = failure.localizedMessage ?: "Der Modelldownload ist fehlgeschlagen."
                    ModelDownloadCoordinator.update(ModelDownloadState.Failed(model, message, partialBytes, total))
                    finish(model, "Download unterbrochen", message)
                }
        }
    ...
    private fun downloadAndVerify(model: WhisperModel) {
        ...
        VerifiedModelDownloader.downloadAndVerify(
            ...
            onVerifying = { bytes ->
                ModelDownloadCoordinator.update(ModelDownloadState.Verifying(model, bytes))
                update(model, bytes, bytes, "Download wird geprüft …")
            }
        )
    }

    private fun update(model: WhisperModel, bytes: Long, total: Long, text: String) {
    ...
    private fun notification(model: WhisperModel, bytes: Long, total: Long, text: String, ongoing: Boolean): Notification {
    ...
    private fun finish(model: WhisperModel, title: String, text: String) {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            getSystemService(NotificationManager::class.java).createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Whisper-Modelldownloads", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    private fun partialFile(model: WhisperModel): File = File(File(filesDir, "models").apply { mkdirs() }, "${model.fileName}.part")

    private fun clearActiveModel() {
        getSharedPreferences(SERVICE_PREFERENCES, MODE_PRIVATE).edit().remove(ACTIVE_MODEL_KEY).apply()
    }

    companion object {
        fun start(context: Context, model: WhisperModel) {
            ContextCompat.startForegroundService(context, Intent(context, ModelDownloadService::class.java).apply {
